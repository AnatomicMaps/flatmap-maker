...
*   At a CLI prompt, and in a suitable directory,
    `download the latest release <https://github.com/AnatomicMaps/flatmap-maker/archive/refs/tags/v1.24.1.tar.gz>`_
    in ``tar.gz`` format and extract it, renaming the top-level directory in the archive to ``flatmapmaker``::

        $ curl -L https://github.com/AnatomicMaps/flatmap-maker/archive/refs/tags/v1.24.1.tar.gz \
        | tar xz -s /v1.24.1.tar.gz/flatmapmaker/
